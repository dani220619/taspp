...
$pdf->MultiCell(31, 0.7, "DATA KAS UMUM", 0, 'C');
...
$pdf->Cell(4.5, 0.8, 'KAS MASUK', 1, 0, 'C');
$pdf->Cell(4.5, 0.8, 'KAS KELUAR', 1, 0, 'C');
...
if (!empty($jurnal)) {
    ...
    foreach ($jurnal as $data) {
        ...
        $pdf->Cell(5, 0.6, $data['keterangan'], 1, 0, 'C');
        $pdf->Cell(4.5, 0.6, 'Rp. ' . number_format($data['debit'], 0, ',', '.'), 1, 0, 'C');
        $pdf->Cell(4.5, 0.6, 'Rp. ' . number_format($data['kredit'], 0, ',', '.'), 1, 0, 'C');
        if ($data['debit'] == 0) {
            $nominal = $data['kredit'];
            $saldo = $saldo - $nominal;
        } else {
...
$pdf->Output("Laporan Kas Umum.pdf", "I");
